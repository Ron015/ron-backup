...
const backup = require("ron-backup");
...
client.on("message", async message => {
    ...
    if (command === "load") {
        ...
        backup.fetch(backupID).then(async () => {
            ...
            try {
                const collected = await message.channel.awaitMessages(m => (m.author.id === message.author.id) && (m.content === "-confirm"), {
                    max: 1,
                    time: 20000,
                    errors: ["time"]
                });
                ...
                if (collected.first().content === "-confirm") {
                    ...
                    // When the backup is loaded, delete it from the server
...
Here are all things that can be restored with `ron-backup`:  
